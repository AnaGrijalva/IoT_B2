...
AWS.config.credentials = new AWS.CognitoIdentityCredentials({
    IdentityPoolId: "ahq026c2b3xc4-ats.iot.us-east-2.amazonaws.com"
});
    ...
    function onScan(err, data) {
        if (err) {
            ...
            stations.forEach(function (id) {
                ...
                var params = {
                    TableName: "iot_b2",
                    ProjectionExpression: "#dtime, temperatura, humedad, vientod, vientoi, lluvia",
                ...
                docClient.query(params, function (err, data) {
                    if (err) {
                        ...
                        document.getElementById('latest-value3').innerHTML = latest.vientod + '°';
                        document.getElementById('latest-value4').innerHTML = latest.vientoi + ' m/s';
                        document.getElementById('latest-value5').innerHTML = latest.lluvia + ' mm/h';
                        ...
                        for (c = 0; c < 5; c++) {
                            document.getElementById('station' + id).innerHTML += '<tr><td>' + actual[c].datetime + '</td><td>' + actual[c].temperatura + '</td><td>' + actual[c].humedad +
                                '</td><td>' + actual[c].vientod + '</td><td>' + actual[c].vientoi + '</td><td>' + actual[c].lluvia + '</td></tr> ';
                        }
                    }
    ...
    function onScan(err, data) {
        if (err) {
            ...
            for (c = 0; c < sizeList; c++) {
                ...
                else if (sensor === "vientod")
                    document.getElementById('id' + sensor).innerHTML += '<tr><td>' + items[c].datetime + '</td><td>' + items[c].id + '</td><td class="text-primary">' + items[c].vientod + '</td></tr> ';
                else if (sensor === "vientoi")
                    document.getElementById('id' + sensor).innerHTML += '<tr><td>' + items[c].datetime + '</td><td>' + items[c].id + '</td><td class="text-primary">' + items[c].vientoi + '</td></tr> ';
                else if (sensor === "lluvia")
                    document.getElementById('id' + sensor).innerHTML += '<tr><td>' + items[c].datetime + '</td><td>' + items[c].id + '</td><td class="text-primary">' + items[c].lluvia + '</td></tr> ';
            }
        }
    };
}